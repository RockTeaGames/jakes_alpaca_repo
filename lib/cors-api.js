class AlpacaCORS {
  ...
  httpRequest(method,args,body=undefined) {
    return fetch(`${this.baseUrl}/v2/${args}`, {
      method: method, 
      mode: 'cors', 
      headers: {
  ...
  dataHttpRequest(method,args,body=undefined) {
    return fetch(`https://data.alpaca.markets/v1/${args}`, {
      method: method, 
      mode: 'cors', 
      headers: {
        "APCA-API-KEY-ID": this.keyId,
  ...
  cancelAllPositions() {
    return this.httpRequest("DELETE",this.argsFormatter("positions",undefined,undefined)).then((resp) => {
      return resp.json();
    }).then((resp) => {
      return resp;
    }).catch((err) => {
